@can('blogs_managment_create')
    <div style="margin-bottom: 10px;" class="row">
        <div class="col-lg-12">
            <a class="btn btn-success" href="{{ route('admin.blogs-managments.create') }}">
...
<div class="card">
    ...
    <div class="card-body">
        <div class="table-responsive">
            <table class=" table table-bordered table-striped table-hover datatable datatable-userBlogsManagments">
                <thead>
                    <tr>
                        <th width="10">
                        ...
                        <tr data-entry-id="{{ $blogsManagment->id }}">
                            ...
                            <td>
                                @can('blogs_managment_show')
                                    <a class="btn btn-xs btn-primary" href="{{ route('admin.blogs-managments.show', $blogsManagment->id) }}">
                                        {{ trans('global.view') }}
                                    </a>
                                @endcan

                                @can('blogs_managment_edit')
                                    <a class="btn btn-xs btn-info" href="{{ route('admin.blogs-managments.edit', $blogsManagment->id) }}">
                                        {{ trans('global.edit') }}
                                    </a>
                                @endcan

                                @can('blogs_managment_delete')
                                    <form action="{{ route('admin.blogs-managments.destroy', $blogsManagment->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('blogs_managment_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('admin.blogs-managments.massDestroy') }}",
...
@endcan
  ...
  let table = $('.datatable-userBlogsManagments:not(.ajaxTable)').DataTable({ buttons: dtButtons })
