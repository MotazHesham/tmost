...
<div class="card">
    ...
    <div class="card-body">
        <div class="table-responsive">
            <table class=" table table-bordered table-striped table-hover datatable datatable-serviceRefugeesLegaRegistrations">
                <thead>
                    <tr>
                        <th width="10">
                        ...
                        <tr data-entry-id="{{ $refugeesLegaRegistration->id }}">
                            ...
                            <td>
                                @can('refugees_lega_registration_show')
                                    <a class="btn btn-xs btn-primary" href="{{ route('admin.refugees-lega-registrations.show', $refugeesLegaRegistration->id) }}">
                                        {{ trans('global.view') }}
                                    </a>
                                @endcan

                                @can('refugees_lega_registration_edit')
                                    <a class="btn btn-xs btn-info" href="{{ route('admin.refugees-lega-registrations.edit', $refugeesLegaRegistration->id) }}">
                                        {{ trans('global.edit') }}
                                    </a>
                                @endcan

                                @can('refugees_lega_registration_delete')
                                    <form action="{{ route('admin.refugees-lega-registrations.destroy', $refugeesLegaRegistration->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@endcan
  ...
  let table = $('.datatable-serviceRefugeesLegaRegistrations:not(.ajaxTable)').DataTable({ buttons: dtButtons })
