        ...
        <div class="col-md-12">
            @can('seminars_subscription_create')
                <div style="margin-bottom: 10px;" class="row">
                    <div class="col-lg-12">
                        <a class="btn btn-success" href="{{ route('frontend.seminars-subscriptions.create') }}">
                                    ...
                                    <tr data-entry-id="{{ $seminarsSubscription->id }}">
                                        ...
                                        <td>
                                            @can('seminars_subscription_show')
                                                <a class="btn btn-xs btn-primary" href="{{ route('frontend.seminars-subscriptions.show', $seminarsSubscription->id) }}">
                                                    {{ trans('global.view') }}
                                                </a>
                                            @endcan

                                            @can('seminars_subscription_edit')
                                                <a class="btn btn-xs btn-info" href="{{ route('frontend.seminars-subscriptions.edit', $seminarsSubscription->id) }}">
                                                    {{ trans('global.edit') }}
                                                </a>
                                            @endcan

                                            @can('seminars_subscription_delete')
                                                <form action="{{ route('frontend.seminars-subscriptions.destroy', $seminarsSubscription->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('seminars_subscription_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('frontend.seminars-subscriptions.massDestroy') }}",
