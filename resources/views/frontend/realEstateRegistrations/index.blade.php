        ...
        <div class="col-md-12">
            @can('real_estate_registration_create')
                <div style="margin-bottom: 10px;" class="row">
                    <div class="col-lg-12">
                        <a class="btn btn-success" href="{{ route('frontend.real-estate-registrations.create') }}">
                                    ...
                                    <tr data-entry-id="{{ $realEstateRegistration->id }}">
                                        ...
                                        <td>
                                            @can('real_estate_registration_show')
                                                <a class="btn btn-xs btn-primary" href="{{ route('frontend.real-estate-registrations.show', $realEstateRegistration->id) }}">
                                                    {{ trans('global.view') }}
                                                </a>
                                            @endcan

                                            @can('real_estate_registration_edit')
                                                <a class="btn btn-xs btn-info" href="{{ route('frontend.real-estate-registrations.edit', $realEstateRegistration->id) }}">
                                                    {{ trans('global.edit') }}
                                                </a>
                                            @endcan

                                            @can('real_estate_registration_delete')
                                                <form action="{{ route('frontend.real-estate-registrations.destroy', $realEstateRegistration->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('real_estate_registration_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('frontend.real-estate-registrations.massDestroy') }}",
